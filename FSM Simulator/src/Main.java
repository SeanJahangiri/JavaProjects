/*
CSC 135 NFA simulator v1.0 -- report errors tdk@csus.edu
...
public class Main {
            ...
            String input)
    {
        ...
        for (int ch = 0; ch < input.length(); ch++) {
            ...
            String inputString = Character.toString(input.charAt(ch));

            for (String state : curStates) {
                if (transition.get(state).get(inputString) != null) {
                    nextCurStates.addAll(transition.get(state).get(inputString));
                }
            }
